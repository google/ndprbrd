...
class RouteTable {
public:
	explicit RouteTable(const QString& proto, std::chrono::duration<int> ttl) : proto_(proto), ttl_(ttl) {}
	void learn(const QHostAddress& addr, const QNetworkInterface& iface) {
		routes_.insert(addr, std::make_pair(iface, std::chrono::steady_clock::now() + ttl_));
	}
	void cleanup() {
		// TODO use min heap
		...
		for (auto it = routes_.begin(); it != routes_.end();) {
			if (it.value().second < now) {
				qDebug() << "deleting" << it.key().toString() << "from" << it.value().first.name();
				QProcess pr;
				pr.start("ip", {"-6", "route", "del", it.key().toString(), "dev", it.value().first.name(), "protocol", proto_});
	...
	void replaceSystem(const QString& addr, QNetworkInterface& iface) {
		QProcess pr;
		pr.start("ip", {"-6", "route", "replace", addr, "dev", iface.name(), "protocol", proto_});
		pr.waitForFinished(-1);
	}
private:
	QHash<QHostAddress, std::pair<QNetworkInterface, std::chrono::steady_clock::time_point /* expiration time */>> routes_;
	QString proto_;
...
class SocketWatcher {
public:
	SocketWatcher(const QNetworkInterface& iface, const PrefixList* prefixes, RouteTable* routes) : iface_(iface), prefixes_(prefixes), routes_(routes) {
		int sock = ::socket(AF_INET6, SOCK_RAW, IPPROTO_ICMPV6);
		if (sock == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't create ICMPv6 socket: " << std::strerror(e) << endl;
			std::exit(1);
		}
		QByteArray ifaceName = iface.name().toUtf8();
		if (::setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE, ifaceName.constData(), ifaceName.size()) == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't bind socket to " << iface.name() << ": " << std::strerror(e) << endl;
			std::exit(1);
		}
		if (::fcntl(sock, F_SETFL, ::fcntl(sock, F_GETFL) | O_NONBLOCK) == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't make socket non-blocking: " << std::strerror(e) << endl;
			std::exit(1);
		}
		notifier_.reset(new QSocketNotifier(sock, QSocketNotifier::Read));
		QObject::connect(notifier_.get(), &QSocketNotifier::activated, [this](int sock) {
			readFrom(sock);
		});
	}
private:
	void readFrom(int sock) {
		...
		ssize_t len = ::recvfrom(sock, buf, sizeof buf, /* flags = */ 0, reinterpret_cast<sockaddr*>(&addr), &addrLen);
		if (len < 24) return;
...
class Tunnel {
public:
	Tunnel(const QString& name, std::vector<QNetworkInterface> interfaces, const PrefixList* prefixes) : interfaces_(std::move(interfaces)), prefixes_(prefixes) {
		...
		if (::ioctl(tap, TUNSETIFF, &ifr) == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't configure TAP interface: " << std::strerror(e) << endl;
			std::exit(1);
		}
		name_ = QString(ifr.ifr_name);
		if (::fcntl(tap, F_SETFL, ::fcntl(tap, F_GETFL) | O_NONBLOCK) == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't make TAP interface non-blocking: " << std::strerror(e) << endl;
			std::exit(1);
		}
		sock_ = ::socket(AF_PACKET, SOCK_RAW, ETH_P_IPV6);
		if (sock_ == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't create raw socket: " << std::strerror(e) << endl;
			std::exit(1);
		}
		// SIOCGIFFLAGS wants a socket. It doesn't matter which exact one.
		if (::ioctl(sock_, SIOCGIFFLAGS, &ifr) == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't get TAP flags: " << std::strerror(e) << endl;
			std::exit(1);
		}
		ifr.ifr_flags |= IFF_UP;
		if (::ioctl(sock_, SIOCSIFFLAGS, &ifr) == -1) {
			int e = errno;
			QTextStream(stderr) << "Error: can't bring TAP interface up: " << std::strerror(e) << endl;
			std::exit(1);
		}
		tapReader_.reset(new QSocketNotifier(tap, QSocketNotifier::Read));
		QObject::connect(tapReader_.get(), &QSocketNotifier::activated, [this](int tap) {
			readFrom(tap);
		});
	}
...
private:
	void readFrom(int tap) {
		...
		// Cheat: don't count checksum fully for every interface, but reuse the existing one
	...
	void sendTo(const QNetworkInterface& iface, sockaddr_ll* ll, char* buf, ssize_t len, uint32_t sum) {
		...
		addrinfo* result;
		if (::getaddrinfo(localIp.toString().toUtf8().constData(), nullptr, &hints, &result) != 0) return;
		if (result->ai_addr->sa_family != AF_INET6) {
			::freeaddrinfo(result);
			return;
		}
		std::memcpy(buf + 22, reinterpret_cast<sockaddr_in6*>(result->ai_addr)->sin6_addr.s6_addr, 16);
		for (int i = 0; i < 16; i+=2) {
			sum += *reinterpret_cast<uint16_t*>(buf + 22 + i);
		}
		...
		*reinterpret_cast<uint16_t*>(buf + 56) = static_cast<uint16_t>(~sum);
		::freeaddrinfo(result);
		::sendto(sock_, buf, len, /* flags = */ 0, reinterpret_cast<sockaddr*>(ll), sizeof *ll);
	}
...
int main(int argc, char** argv) {
	...
	parser.setApplicationDescription("NDP Routing Bridge Daemon\n\nExample: " + app.applicationName() + " --interface=eth1 --interface=eth2 --prefix=2001:db8:1:2::/64");
	parser.addHelpOption();

	QCommandLineOption interfaceOpt("interface", "LAN interfaces where hosts are discovered (multiple allowed)", "iface");
	parser.addOption(interfaceOpt);

	QCommandLineOption prefixOpt("prefix", "Subnets /64 which should be routed by ndprbrd (multiple allowed)", "prefix");
	parser.addOption(prefixOpt);

	QCommandLineOption expireOpt("expire", "How long should discovered routes stay when not used, default: 600", "secs", "600");
	parser.addOption(expireOpt);

	QCommandLineOption protoOpt("protocol", "Protocol num for routing table, as known by kernel, default: 100", "num", "100");
	parser.addOption(protoOpt);

	QCommandLineOption pendulumOpt("pendulum", "(not recommended) Rapidly switch default route between LAN interfaces instead of creating TAP interface");
	parser.addOption(pendulumOpt);

	QCommandLineOption tunOpt("tun", "Name of TAP interface to create, default: ndprbrdN", "iface", "ndprbrd%d");
	...
	RouteTable routes(parser.value(protoOpt), std::chrono::seconds(parser.value(expireOpt).toInt()));
	...
	for (const QString& iface : parser.values(interfaceOpt)) {
		...
		if (!interface.isValid()) {
			QTextStream(stderr) << "Error: interface " << iface << " is not valid" << endl;
			return 1;
		}
		// Fill the table with existing routes
		QProcess pr;
		pr.start("ip", {"-6", "route", "show", "dev", interface.name(), "protocol", parser.value(protoOpt)});
		pr.waitForFinished(-1);
		for (const QString& line : QString::fromUtf8(pr.readAllStandardOutput()).split('\n', QString::SkipEmptyParts)) {
	...
	QTimer expirator;
	QObject::connect(&expirator, &QTimer::timeout, [&]() {
		routes.cleanup();
	});
	...
	if (parser.isSet(pendulumOpt)) {
		...
		tunnel.reset(new Tunnel(parser.value(tunOpt), std::move(interfaces), &prefixes));
		QNetworkInterface tun = QNetworkInterface::interfaceFromName(tunnel->name());
		for (const QString& prefix : prefixesStr) {
